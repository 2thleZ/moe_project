...
def test_dispatch_coverage():
    """verify dispatch coverage across tokens and experts."""
    num_tokens = 10
    hidden_dim = 16
    top_k = 2
    num_experts = 4
    ...
    # verify total dispatched rows match num_tokens * top_k
    assert dispatched_x.shape[0] == num_tokens * top_k
    assert count.sum() == num_tokens * top_k

def test_empty_experts():
    """validate dispatch when experts are empty."""
    num_tokens = 8
    hidden_dim = 16
    top_k = 1
    num_experts = 4
    
    # force all tokens to target expert 2
    x = torch.randn(num_tokens, hidden_dim)
...
def test_dispatch_combine_correctness():
    """verify that combine successfully inverts dispatch with identity mapping."""
    num_tokens = 5
    hidden_dim = 8
    top_k = 2
    num_experts = 3
    ...
    x = torch.randn(num_tokens, hidden_dim)
    # define fixed assignments for deterministic testing
    experts = torch.tensor([
        [0, 1],
        [1, 2],
        [0, 2],
        [2, 2], # validate duplicate expert handling
        [0, 0]
    ])
...
def test_routing_modes():
    """verify uniform and skewed routing distribution behavior."""
    num_tokens = 32
    hidden_dim = 16
    ...
    # validate skewed distribution (worst case)
...
def test_triton_vs_pt_correctness():
    """verify that triton kernel output matches pytorch baseline."""
